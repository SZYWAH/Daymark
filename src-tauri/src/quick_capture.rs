...
use windows_sys::Win32::{
    ...
    UI::{
        ...
        WindowsAndMessaging::{
            ...
            TranslateMessage, KBDLLHOOKSTRUCT, MSG, WH_KEYBOARD_LL, WM_KEYDOWN, WM_SYSKEYDOWN,
        },
    },
};
...
static QUICK_CAPTURE_DESTROY_RECONCILE_SUPPRESSIONS: OnceLock<Mutex<HashSet<String>>> =
    OnceLock::new();
        ...
        thread::spawn(move || {
            ...
            run_quick_capture_on_main(&fallback_app, move |app| {
                if current_panel_open_token() != token
                    || !matches!(
                        quick_capture_state(),
                        QuickCaptureState::PanelOpen | QuickCaptureState::PanelDetached
                    )
...
fn start_quick_capture_escape_hook(app: AppHandle) {
    ...
    thread::spawn(move || unsafe {
        ...
        let hook = SetWindowsHookExW(WH_KEYBOARD_LL, Some(quick_capture_keyboard_hook), module, 0);
        if hook.is_null() {
...
fn remember_quick_capture_monitor(app: &AppHandle) -> Result<(), String> {
    ...
    *quick_capture_monitor_store()
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner()) =
        Some(QuickCaptureMonitor::from_monitor(&monitor));
    Ok(())
}
...
fn quick_capture_geometry(
    ...
    let monitor =
        stored_monitor.ok_or_else(|| "Unable to read monitor information.".to_string())?;
    let x = anchored_x(
        monitor.logical_x,
...
fn quick_capture_window(
    ...
    if let Some(window) = app.get_webview_window(label) {
        let keep_detached_position = label == QUICK_CAPTURE_PANEL_LABEL
            && quick_capture_state() == QuickCaptureState::PanelDetached;
...
fn prewarm_quick_capture_window(app: &AppHandle) -> Result<(), String> {
    if app
        .get_webview_window(QUICK_CAPTURE_HOTZONE_LABEL)
        .is_some()
    {
        return Ok(());
    }
    ...
    let (position, size) =
        quick_capture_geometry(app, QUICK_CAPTURE_HOT_WIDTH, QUICK_CAPTURE_HOT_HEIGHT)?;
        ...
        loop {
            ...
            if quick_capture_hotzone_reopen_suppressed() {
                ...
                thread::sleep(Duration::from_millis(
                    QUICK_CAPTURE_HOTZONE_POLL_INTERVAL_MS,
                ));
                continue;
            }
...
pub(crate) fn show_quick_capture_hotzone_for_hidden_main_impl(
    app: &AppHandle,
) -> Result<(), String> {
    show_quick_capture_hotzone_impl_with(app, true)
}

pub(crate) fn show_quick_capture_hotzone_impl_with(
    app: &AppHandle,
    force_hidden_main: bool,
) -> Result<(), String> {
    ...
    let _ = hotzone.set_ignore_cursor_events(true);
    hotzone
        .emit("quick-capture:hotzone-probe", hotzone_token)
        .ok();
...
pub(crate) fn hide_quick_capture_panel_impl(
    app: &AppHandle,
    token: Option<u64>,
) -> Result<bool, String> {
...
pub(crate) fn return_quick_capture_to_hotzone_impl(
    app: &AppHandle,
    token: Option<u64>,
) -> Result<bool, String> {
    if !panel_token_is_current(token) {
...
pub(crate) fn reconcile_quick_capture_window_destroyed(app: &AppHandle, label: &str) {
    ...
    if label == QUICK_CAPTURE_HOTZONE_LABEL
        && quick_capture_state() == QuickCaptureState::HotzoneVisible
    {
        if quick_capture_degraded() {
            set_quick_capture_state(QuickCaptureState::Degraded);
    ...
    let dispatched = app.run_on_main_thread(move || {
        ...
        if panel_active && !panel_visible {
            ...
            if visible && !minimized {
                ...
                if hotzone_visible {
                    if let Some(hotzone) =
                        app_handle.get_webview_window(QUICK_CAPTURE_HOTZONE_LABEL)
                    {
                        let token = current_hotzone_open_token();
                        if token != 0 {
                            hotzone.emit("quick-capture:hotzone-probe", token).ok();
        ...
        if visible {
            if focused && !panel_visible && !panel_active && !panel_transitioning && hotzone_visible
            {
                set_quick_capture_state(QuickCaptureState::MainVisible);
                hide_quick_capture_windows(&app_handle);
                return;
...
#[tauri::command]
pub(crate) fn open_main_from_quick_capture(
...
#[tauri::command]
pub(crate) fn collapse_quick_capture(
    window: WebviewWindow,
    app: AppHandle,
) -> Result<bool, String> {
...
#[tauri::command]
pub(crate) fn show_quick_capture_hotzone(
    window: WebviewWindow,
    app: AppHandle,
) -> Result<(), String> {
    ensure_main_window(&window)?;
...
#[tauri::command]
pub(crate) fn hide_quick_capture_panel(
    window: WebviewWindow,
    app: AppHandle,
    token: Option<u64>,
) -> Result<bool, String> {
...
#[tauri::command]
pub(crate) fn quick_capture_window_ready(
    window: WebviewWindow,
    label: String,
    token: Option<u64>,
) -> Result<(), String> {
...
#[tauri::command]
pub(crate) fn set_quick_capture_saving(
    window: WebviewWindow,
    saving: bool,
    token: Option<u64>,
) -> Result<(), String> {
...
#[tauri::command]
pub(crate) fn notify_quick_capture_saved(
    window: WebviewWindow,
    app: AppHandle,
    token: Option<u64>,
) -> Result<bool, String> {
...
mod tests {
    ...
    fn anchored_positions_use_stable_edge_margins() {
        assert_eq!(
            anchored_x(0.0, 1_920.0, 760.0, QuickCaptureAnchor::Left, 24.0),
            24.0
        );
        assert_eq!(
            anchored_x(0.0, 1_920.0, 760.0, QuickCaptureAnchor::Center, 24.0),
            580.0
        );
        assert_eq!(
            anchored_x(0.0, 1_920.0, 760.0, QuickCaptureAnchor::Right, 24.0),
            1_136.0
        );
    }
    ...
    fn panel_center_selects_left_center_and_right_thirds() {
        assert_eq!(
            anchor_for_panel_center(0.0, 1_920.0, 300.0),
            QuickCaptureAnchor::Left
        );
        assert_eq!(
            anchor_for_panel_center(0.0, 1_920.0, 960.0),
            QuickCaptureAnchor::Center
        );
        assert_eq!(
            anchor_for_panel_center(0.0, 1_920.0, 1_700.0),
            QuickCaptureAnchor::Right
        );
    }
