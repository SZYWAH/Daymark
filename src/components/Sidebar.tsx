...
import { getChildFolders } from "../lib/folders";
  ...
  if (libraryActive) {
    return (
      <aside className="sidebar-surface overflow-hidden" style={sidebarStyle}>
        ...
        <LibraryGlobalNav activeView={activeView} onSelectView={onSelectView} />
...
function LibraryGlobalNav({
  ...
  return (
    <nav className="mb-4 grid grid-cols-5 gap-1.5 rounded-[10px] border border-line/70 bg-panel/45 p-1">
      {entries.map(({ label, icon: Icon, active, onClick }) => (
        <button
          key={label}
          className={`mobile-library-nav-button flex h-11 min-w-0 flex-col items-center justify-center gap-0.5 rounded-[8px] text-ink/50 transition hover:bg-surface/80 hover:text-ink ${
            active ? "mobile-library-nav-button-active text-ink" : ""
          }`}
          onClick={onClick}
          title={label}
        >
...
function FolderRow({
  ...
  return (
    <div>
      <div
        className={`group relative flex h-8 items-center gap-1 rounded-[8px] pr-1 text-sm ${
          active ? "nav-row-active" : "nav-row"
        }`}
